...
- A diff adds no comments. That is the expected outcome and not a starting position to argue up from. The two exceptions below are the entire set, and no reasoning, however good, produces a third.
- The exceptions, one line each: (1) a workaround for a named external defect or a documented behavior that contradicts the obvious reading, where the comment names the system; (2) code that looks wrong and that a reader would confidently correct, where the comment names what the correction breaks. Being subtle, clever, or hard to follow is not a trigger for either.
- Never write: a comment opening with the symbol name, a doc comment on any symbol, a section or step marker over a block, a restatement of the line below, why you chose this approach over another, or anything added because the file around you is commented. Change history, a rejected alternative, and the case for the code itself belong in the commit message or the pull request.
...
- Never add a comment to code you did not write, and never remove a comment that was already there unless the request names it. Comment density in the file around you is a fact about its history, not a standard to match.
